...
z_mat = matrix(proposed$y_pred, length(unique(x_pred[,1])), length(unique(x_pred[,2])))
filled.contour(unique(x_pred[,1]), unique(x_pred[,2]), z_mat, 
               levels = seq(min(proposed$y_pred),max(proposed$y_pred), length = 20), nlevels = 20,
               col = terrain.colors(22)[1:20],
               main = "Test Predictions")


par(mfrow = c(2,3))
plot.ts(proposed$Jvec[,1])
plot.ts(proposed$Jvec[,2])
